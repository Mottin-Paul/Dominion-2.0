...
public class Game {
	 ...
	 * @param kingdomStacks
	 *            liste de piles de réserve à utiliser correspondant aux
	 *            cartes "royaume" à utiliser dans la partie, auxquelles le
	 *            constructeur doit ajouter les piles "communes": - 60 Copper -
	 *            40 Silver - 30 Gold - 8 (si 2 joueurs) ou 12 (si 3 ou 4
	 *            joueurs) Estate, Duchy et Province * - 10 * (n-1) Curse où n
	...
	/**
	 * Renvoie une représentation de l'état de la partie sous forme d'une
	 ...
	 * @return la carte trouvée dans la réserve ou {@code null} si aucune
	 *         carte ne correspond
	 */
		...
		for (int i = 0; i < this.supplyStacks.size(); i++) {
			for (int j = 0; j < this.supplyStacks.get(i).size(); j++) {
				if (this.supplyStacks.get(i).get(j).getName().equals(card_name)) {
					...
					this.supplyStacks.get(i).remove(
							this.supplyStacks.get(i).get(j));
	 ...
	 * 
	 * @return un booléen indiquant si la partie est terminée, c'est-à-dire
	 *         si au moins l'unedes deux conditions de fin suivantes est vraie -
	 *         3 piles ou plus de la réserve sont vides - la pile de Provinces
	 *         de la réserve est vide (on suppose que toute partie contient une
	 ...
	 * 
	 * Cette méthode exécute les tours des joueurs jusqu'à ce que la partie
	 * soit terminée. Lorsque la partie se termine, la méthode affiche le
	 * score final et les cartes possédées par chacun des joueurs.
	 */
	public void run() {
	 ...
	 * 
	 * C'est cette méthode qui doit être appelée à chaque fois qu'on veut
	 * lire l'entrée clavier de l'utilisateur (par exemple dans Player.choose),
	 * ce qui permet de n'avoir qu'un seul Scanner pour tout le programme
	 * 
